...
class TotalCostAgent(BaseAgent):
    ...
    async def process(self, **kwargs) -> Dict[str, Any]:
        try:
            ...
            return AgentResponse(success=True, data=cost_data, error=None).to_dict()
    ...
    def _calculate_per_person_costs(
        self, total_costs: Dict[str, float], num_passengers: int
    ) -> Dict[str, float]:
    ...
    def generate_cost_optimization_tips(self, costs: Dict[str, Any]) -> List[str]:
        ...
        if costs["fuel"] > costs["total_cost"] * 0.3:
            tips.append(
                "Consider carpooling or using public transport for parts of the journey"
            )

        if costs["accommodation"] > costs["total_cost"] * 0.4:
            tips.append(
                "Look for alternative accommodation options or consider camping"
            )

        return tips

    def analyze_cost_patterns(self, costs: Dict[str, Any]) -> Dict[str, Any]:
