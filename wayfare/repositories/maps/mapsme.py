...
class MapsMeRepository(BaseMapsRepository):
    ...
    async def _search_mapsme(
        self, query: str, location: Optional[GeoLocation] = None
    ) -> Dict:
    ...
    async def geocode(self, address: str) -> GeoLocation:
        ...
        return GeoLocation(
            latitude=result["lat"], longitude=result["lon"], address=address
        )
    ...
    async def get_route(
        ...
        if waypoints:
            params["waypoints"] = [
                {"lat": w.latitude, "lon": w.longitude} for w in waypoints
            ]
