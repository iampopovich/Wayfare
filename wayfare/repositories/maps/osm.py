...
class OSMRepository(BaseMapsRepository):
    ...
    def _search_osm(self, query: str, location: Optional[GeoLocation] = None) -> Dict:
        ...
        if location:
            gdf = ox.geometries_from_point(
                (location.latitude, location.longitude), tags={"name": query}
            )
        else:
    ...
    async def geocode(self, address: str) -> GeoLocation:
        ...
        return GeoLocation(latitude=location[0], longitude=location[1], address=address)

    async def reverse_geocode(self, location: GeoLocation) -> str:
        """Convert coordinates to address using OSM Nominatim."""
        address = ox.reverse_geocode((location.latitude, location.longitude))
        return address
    ...
    async def get_route(
        ...
        G = ox.graph_from_point(
            ...
            dist=max(
                [
                    ox.distance.great_circle_vec(points[0][0], points[0][1], p[0], p[1])
                    for p in points
                ]
            )
            + 1000,
