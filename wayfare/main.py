...
async def log_requests(request: Request, call_next):
    ...
    try:
        ...
        logger.info(
            f"Request completed - {method} {path} - Took {process_time:.2f}s - Status {response.status_code}"
        )
        ...
        logger.error(
            f"Error processing request {method} {path}: {str(e)}", exc_info=True
        )
        return JSONResponse(
    ...
    return JSONResponse(
        {
            ...
            "endpoints": {
                ...
                "travel": {"plan_route": "/api/v1/travel/route"},
            },
            "documentation": "/docs",
...
if __name__ == "__main__":
    ...
    # Run the server
    uvicorn.run(
        "main:app", host="0.0.0.0", port=port, reload=True  # Required for Render
    )
