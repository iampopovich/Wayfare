...
async def plan_route(
    request: TravelRequest, travel_service: TravelService = Depends(get_travel_service)
):
    """
    ...
    try:
        logger.info(
            f"Received travel request: origin={request.origin}, destination={request.destination}, "
        ...
        logger.info(
            f"Successfully planned route from {request.origin} to {request.destination}"
        )
        return result
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error while planning route"
        )
