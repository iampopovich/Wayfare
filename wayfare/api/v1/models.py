...
class SeaTransportPreference(BaseModel):
    ...
    cabin_class: Optional[str] = Field(None, description="Preferred cabin class for cruises/ferries")
    max_travel_time: Optional[int] = Field(None, description="Maximum travel time in hours")
    preferred_companies: Optional[List[str]] = Field(None, description="Preferred shipping/cruise companies")
    require_onboard_accommodation: bool = Field(False, description="Whether overnight accommodation on board is required")
...
class OvernightPreference(BaseModel):
    ...
    min_rating: Optional[float] = Field(None, ge=0, le=5, description="Minimum rating for accommodation")
    include_onboard_accommodation: bool = Field(False, description="Whether to include accommodation on sea transport")
...
class SearchRequest(BaseModel):
    ...
    budget: Optional[BudgetRange] = Field(None, description="Budget constraints for the search")
    ...
    sea_transport_preference: Optional[SeaTransportPreference] = Field(
        None,
        description="Sea transport specific preferences"
    )
    overnight_stay: Optional[OvernightPreference] = Field(
        None,
        description="Overnight stay preferences"
    )
...
class RouteRequest(BaseModel):
    ...
    sea_transport_preference: Optional[SeaTransportPreference] = Field(
        None,
        description="Sea transport specific preferences when using sea transport"
    )
    include_overnight_stays: bool = Field(
        False,
        description="Whether to include overnight stay recommendations"
    )
    budget_per_day: Optional[float] = Field(
        None,
        description="Maximum budget per day for the route"
    )
...
class AccommodationSearchRequest(BaseModel):
    ...
    transportation_to_city_center: Optional[List[TransportationType]] = Field(
        None,
        description="Preferred transportation types to city center"
    )
...
class RouteSegment(BaseModel):
    ...
    port_details: Optional[dict] = Field(
        None,
        description="Port information for sea transport segments"
    )
